...
bool district_solver::solve_for_bigness()
{
  ...
  while(true) {
    ...
    if(good) return true;
    if(!bigness_steps[smallest_index]) return false;
    ...
    if(good) return true;
    if(!bigness_steps[largest_index]) return false;
...
void district_solver::read_data(istream &in)
{
  ...
  for(i=0;i<pop.size();i++) {
    ...
    if(fullreps>0) {
      ...
      cout << "County subdivision at " << pop[i].latitude << ',' <<
	pop[i].longitude << " has enough people for " << fullreps <<
	" full districts" << endl;
    }
  }
  dist.resize(nreps);
